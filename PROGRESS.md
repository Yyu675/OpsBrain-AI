...
| **T20** | **S2-1：诊断编排器**（路线图 §6.1，报告 106 **已产**） | 骨架批次完成：V4 sys_diagnosis_session（唯一索引防重） + DiagnosisOrchestrator（4/8/100 异步池 + TRACE 串链） + DiagnosisSessionRepository（幂等提交 + 成败兜底） + 4 个测试；触发钩子下批 | 2026-09-07 |
