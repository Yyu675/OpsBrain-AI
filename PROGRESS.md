...
| **T21** | **S2-2：根因假设与置信度**（路线图 §6.2） | 批次 A 完成：Hypothesis model + ConfidenceEngine（务实公式，路线图的五个系数一项不落）+ V5 hypothesis 表 + 5 个单元测试；批次 B（LLM 推理 prompt 设计 + 假设解析入库）排下一轮 | 2026-09-07 |
