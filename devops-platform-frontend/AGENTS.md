...
- **禁止硬编码色值**。一律使用 `assets/styles/theme.css` 的语义令牌
  （`--surface-*` / `--text-*` / `--border-*` / `--brand*` / `--success|warning|danger|info`
  / `--sev-p0..p3` / `--chart-1..6`）。
  硬编码的颜色在暗色下**不会变**，会造成「卡片变暗了但里面的标签还是浅色」的破碎感。
  > 存量 639 处硬编码正在分批清理，改到相关文件时请顺手替换，不得新增。
- **间距/圆角/字号也走令牌**（`--space-*` / `--radius*` / `--text-*`）。
  它们受密度轴与圆角轴控制，写死会让主题切换在该处失效。
- **动效必须用 `--duration-*` 与 `--ease-*`**。
  直接写 `0.2s ease` 会绕过 `prefers-reduced-motion` 归零，
  对前庭功能障碍用户造成眩晕（WCAG 要求）。
- 新增组件前先看 **`/design-system`** 展示页，照其样式与密度写，不要各页自创一套。
- 改动令牌后，**先在 `/design-system` 逐轴验收**（明暗 × 配色 × 圆角 × 密度）。
- 旧变量名（`--color-*`）由 `theme-bridge.css` 映射到新令牌，**新代码不要再用旧名**。
