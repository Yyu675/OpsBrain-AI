# CLAUDE.md §6 技术决策记录全量归档（6.1 ~ 6.59）

> **定位**（批 69 / 报告 170 收尾，2026-09-09）：本文件是根目录 `CLAUDE.md` 第六章
> **6.1~6.59 全部决策记录的逐字归档**。CLAUDE.md 瘦身后正文只保留决策索引表，
> 完整内容（背景/方案对比/实施账/契约/验证证据）在此。
>
> **锚点不变**：各节标题 `### 6.x` 与原文逐字一致——外部文档中
> 「CLAUDE.md §6.5」「CLAUDE.md 6.25」类引用，从本归档件的同名锚点仍可达。
>
> **为什么归档不删**（批 68 归档 A 判例）：这些决策记录是勾稽审计链的一环，
> PRD/实施记录/核查报告按节号引用 14+ 处；删除=断链。它们已完成历史使命
> 但仍是「为什么当初这么做」的唯一现场。
>
> 后续新增技术决策写入 `docs/09-decisions/`，不再追加 6.x 编号。
