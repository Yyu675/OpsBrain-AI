...
> ⚠️ 本地无 Maven，**这 9 例只能由 CI 验证**。若仍失败，
> 断言消息里已带上 status / contentType / bodyLen / body 前 500 字符，
> 足以区分「401」「404」「流为空」三种情况——这正是上一轮欠缺的诊断能力。
...
1. **视 CI 结果决定 SSE 后续**。若仍失败，断言消息已能区分 401/404/空流三类，
   一轮即可定位；若通过，则本项目最后一处 `@Disabled` 清零。
