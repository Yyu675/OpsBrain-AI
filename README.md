...
- ⚠️ **不是全自动自愈系统**。L4 受控自愈**链路已通**(告警→策略→三闸→
  Saga 补偿→回放留痕),但执行器面当前为 **Mock/沙箱轨 + K8s 只读诊断**;
  「低危故障 AI 真动生产」仍属 roadmap（真执行器接挂清单挂账中）。
...
| **L3** | 人机协同：告警驱动的诊断闭环 — **取证 → 假设 → 置信度 → 回放** | ✅ **可用**（规则基线永远可用 / LLM 可插拔） |
| **L4** | 受控自愈：**治理门 → 演练 → 审批回路 → 执行 → 快照撤销** | ✅ **链路全通**：真实 K8s 执行器 ×2（重启 Pod/扩缩容）+ Mock ×2 已注册；真窗干跑在联合测试清单 T9/T10 |
| **L5** | 端到端自治 | ⏳ 未启动 |
